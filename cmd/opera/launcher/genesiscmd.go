...
func exportGenesis(ctx *cli.Context) error {
	...
	if mode != "full" && mode != "mpt" && mode != "none" {
		return errors.New("--export.evm.mode must be one of {full, mpt, none}")
	}
	...
	fromBlock := idx.Block(0)
	{
		...
		for i := to; i >= from; i-- {
			...
			if er == nil {
				return fmt.Errorf("epoch %d was not found", i)
			}
	...
	}
	{
		...
		for i := toBlock; i >= fromBlock; i-- {
			...
			if br == nil {
				return fmt.Errorf("block %d was not found", i)
			}
