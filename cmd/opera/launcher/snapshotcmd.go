...
func pruneState(ctx *cli.Context) error {
	...
	if ctx.Bool(PruneExactCommand.Name) {
		lset, closer, err := evmpruner.NewLevelDBSet(path.Join(tmpDir, "non-pruned-keys"))
		if err != nil {
		...
		bloom, err = evmpruner.NewProbabilisticSet(ctx.Uint64(utils.BloomFilterSizeFlag.Name))
		if err != nil {
			log.Error("Failed to create state bloom", "err", err)
			return err
		}
